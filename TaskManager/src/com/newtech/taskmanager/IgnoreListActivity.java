...
public class IgnoreListActivity extends ListActivity implements OnClickListener {
	/** all marked flag. */
							...
							if (mAdapter != null) {
								new Thread(new Runnable() {
									...
									public void run() {
										...
										IgnoreListActivity.this.runOnUiThread(new Runnable() {
											@Override
											public void run() {
												updateButtonState();
												setEmptyText();
...
//		}
	...
	private void deleteOneItem(long id) {
		try {
			mContentResolver.delete(Constants.IGNORE_LIST_URI, Constants._ID
					+ "=" + id, null);
	...
	class LoadDataTask extends AsyncTask<Void, Void, Cursor> {
		...
		protected Cursor doInBackground(Void... params) {
			try {
				return mContentResolver.query(Constants.IGNORE_LIST_URI,
						Utils.getIgnoreProject(), null, null, null);
			} catch (SQLiteException e) {
				e.printStackTrace();
			}
			return null;
